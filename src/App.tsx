...
const siteData = {
  ...
  home: {
    title: "2025年度复盘",
    avatar:
      "https://mujiasuotuchuang1-1422708709.cos.ap-shanghai.myqcloud.com/20260505214454584.png",
    name: "木木",
  ...
  portfolio: {
    ...
    projects: [
      // ===== 先锋 (48个) =====
      {
        ...
        name: "先锋项目1",
        group: "先锋",
        img: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=500&auto=format&fit=crop",
      },
      {
        id: 2,
        name: "先锋项目2",
  ...
  assets: {
    banner: {
      ...
      longImageSrc:
        "https://mujiasuotuchuang1-1422708709.cos.ap-shanghai.myqcloud.com/20260505224810577.jpg",
    },
    sop: {
      title: "拼装翻页万年历SOP",
      steps: [
    ...
    <div className="text-center space-y-8 w-full max-w-5xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-4">
        {siteData.home.stats.map((stat) => (
          <div
            key={stat.id}
            className="bg-white border-[4px] border-black rounded-2xl overflow-hidden shadow-[8px_8px_0_0_#000] transform transition-transform hover:-translate-y-2 hover:-translate-x-1 hover:shadow-[12px_12px_0_0_#000] flex flex-col"
          >
            <div
              className={`h-6 w-full border-b-[4px] border-black bg-gradient-to-r ${stat.colorClass}`}
            ></div>
...
const PortfolioView = () => {
  const [activeFilter, setActiveFilter] = useState(
    siteData.portfolio.categories[0]
  );
  const [viewMode, setViewMode] = useState("grid");
  const [selectedImage, setSelectedImage] = useState(null);

  const filteredProjects =
    activeFilter === siteData.portfolio.categories[0]
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 bg-white p-4 border-[3px] border-black shadow-[6px_6px_0_0_#000] rounded-2xl">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`p-2 rounded-xl border-[3px] border-black transition-all ${
              viewMode === "grid"
                ? "bg-black text-white"
                : "bg-white text-black shadow-[3px_3px_0_0_#000] hover:bg-[#FF90E8]"
            }`}
          >
          ...
          <button
            ...
            className={`p-2 rounded-xl border-[3px] border-black transition-all ${
              viewMode === "list"
                ? "bg-black text-white"
                : "bg-white text-black shadow-[3px_3px_0_0_#000] hover:bg-[#26D0CE]"
            }`}
          >
        ...
        {filteredProjects.map((project, idx) => {
          ...
          return (
            <div
              ...
              } bg-white border-[4px] border-black shadow-[8px_8px_0_0_#000] rounded-2xl overflow-hidden transition-transform duration-300 ${rotateClass} hover:-translate-y-2 hover:-translate-x-1 hover:shadow-[12px_12px_0_0_#000]`}
            >
              <div
                className={`${
                  viewMode === "list"
                    ? "w-32 h-24 md:w-56 md:h-36 shrink-0 border-[3px] border-black rounded-xl"
                    : "w-full aspect-[4/3] border-b-[4px] border-black"
                } overflow-hidden bg-[#FFD074] relative cursor-pointer group-hover:opacity-90 transition-opacity`}
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                  <span className="bg-black text-white font-black px-3 py-1 border-2 border-white rounded shadow-[2px_2px_0_0_#fff] transform scale-90 group-hover:scale-100 transition-transform">
                    放大
                  </span>
                </div>
              </div>
              ...
              <div
                className={`p-5 ${
                  viewMode === "list"
                    ? "flex-1 flex justify-between items-center bg-transparent"
                    : "bg-white"
                }`}
              >
          ...
          <div className="relative h-80 min-w-[800px] flex items-end justify-between pt-10 pl-12 border-l-[4px] border-b-[4px] border-black mt-10">
            <div className="absolute -left-12 top-0 bottom-0 flex flex-col justify-between text-black font-black text-sm pb-0">
              {yAxisLabels.map((val, idx) => (
                <span
                  key={idx}
                  className="bg-white border-2 border-black px-2 py-0.5 rounded-lg shadow-[2px_2px_0_0_#000] -rotate-6 translate-y-2"
                >
                  {val}
                </span>
              ))}
            ...
            {sortedData.map((data, i) => (
              <div
                key={i}
                className="flex flex-col items-center relative group flex-1 px-1 transition-all duration-500"
              >
                ...
                <div className="absolute -top-20 opacity-0 group-hover:opacity-100 transition-all bg-white border-[4px] border-black text-black text-sm p-4 shadow-[6px_6px_0_0_#000] z-10 font-black whitespace-nowrap rounded-2xl transform scale-90 group-hover:scale-100">
                  <div className="mb-2 text-lg text-[#26D0CE] stroke-black">
                    {data.name}
                  </div>
                  {(sortBy === null || sortBy === "sample") && (
                    <div>
                      {legend.sample}: {data.sample}
                    </div>
                  )}
                  {(sortBy === null || sortBy === "order") && (
                    <div>
                      {legend.order}: {data.order}
                    </div>
                  )}
                </div>
      ...
      {sections.map((section, sIdx) => (
        <section key={section.id} className="relative">
          <div className="flex items-center gap-6 mb-10">
            <div
              className={`w-12 h-12 ${section.themeColor} border-[4px] border-black rounded-full shadow-[4px_4px_0_0_#000]`}
            ></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {section.items.map((item, index) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="group bg-white border-[4px] border-black p-8 rounded-3xl shadow-[8px_8px_0_0_#000] hover:-translate-y-2 hover:-translate-x-1 hover:shadow-[12px_12px_0_0_#000] transition-all"
                  >
                    <div className="flex justify-between items-start mb-10">
                      <div
                        className={`w-14 h-14 bg-white border-[3px] border-black rounded-2xl flex items-center justify-center text-black shadow-[4px_4px_0_0_#000] group-hover:rotate-12 transition-transform`}
                      >
                        {ItemIcon && <ItemIcon size={28} strokeWidth={2.5} />}
                      </div>
                      <span
                        className={`text-2xl font-black px-3 py-1 border-[3px] border-black rounded-xl shadow-[4px_4px_0_0_#000] -rotate-6 ${
                          index % 2 === 0 ? "bg-[#FF90E8]" : "bg-[#FFD074]"
                        }`}
                      >
                        #{index + 1}
                      </span>
            ...
            <div className="bg-[#f4f1ea] border-[4px] border-black p-12 min-h-[350px] flex items-center justify-center rounded-3xl shadow-[inset_8px_8px_0_rgba(0,0,0,0.05)]">
              <div className="text-center bg-white p-8 border-[4px] border-black border-dashed rounded-2xl shadow-[8px_8px_0_0_#000] -rotate-2">
                <Layers
                  ...
                  strokeWidth={2.5}
                />
                <p className="font-black text-xl text-black uppercase tracking-widest">
                  {section.placeholderText}
                </p>
              </div>
            </div>
...
const AssetsView = () => {
  ...
  return (
    <div className="animate-in fade-in duration-500 space-y-16">
      <section>
        <div
          className={`border-[4px] border-black shadow-[12px_12px_0_0_#000] rounded-3xl p-10 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden ${banner.themeColor}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-8">
        <section className="bg-white border-[4px] border-black rounded-3xl p-8 shadow-[8px_8px_0_0_#000]">
          <h2 className="text-3xl font-black text-black mb-10 tracking-tight inline-block border-b-[4px] border-black pb-2">
            {sop.title}
          </h2>
          <div className="space-y-6">
            {sop.steps.map((step, idx) => (
              <div
                key={step.id}
                className="flex gap-6 items-start p-5 bg-[#f4f1ea] border-[3px] border-black rounded-2xl shadow-[4px_4px_0_0_#000] hover:-translate-y-1 hover:shadow-[6px_6px_0_0_#000] transition-all"
              >
                ...
                <div>
                  <h4 className="text-black font-black text-xl">
                    {step.title}
                  </h4>
                  <p className="text-black/70 font-bold mt-2 text-sm">
                    {step.desc}
                  </p>
                </div>
              </div>
        ...
        <section>
          <h2 className="text-3xl font-black text-black mb-10 tracking-tight bg-[#FF90E8] border-[4px] border-black px-6 py-3 inline-block rounded-2xl shadow-[6px_6px_0_0_#000] rotate-2">
            {plugins.title}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {plugins.items.map((plugin) => (
              <div
                key={plugin.id}
                className="group border-[4px] border-black p-6 rounded-2xl bg-white shadow-[6px_6px_0_0_#000] hover:-translate-y-2 hover:-translate-x-1 hover:shadow-[10px_10px_0_0_#000] transition-all cursor-pointer flex flex-col justify-between min-h-[160px]"
              >
    ...
    <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[100] flex flex-col items-end">
        ...
        <div className="mb-4 bg-white border-[4px] border-black p-4 rounded-3xl shadow-[8px_8px_0_0_#000] animate-in slide-in-from-bottom-4 fade-in duration-200 transform origin-bottom-right -rotate-2">
          ...
          <div className="bg-[#f4f1ea] border-[3px] border-black rounded-xl p-2 shadow-[inset_4px_4px_0_rgba(0,0,0,0.05)]">
            <img
              src={qrCodeImg}
              alt="QR Code"
              className="w-32 h-32 md:w-40 md:h-40 rounded-lg mix-blend-multiply"
            />
          </div>
        </div>
      )}
      <button
        ...
        className={`w-14 h-14 md:w-16 md:h-16 border-[4px] border-black rounded-full flex items-center justify-center shadow-[6px_6px_0_0_#000] hover:-translate-y-1 hover:shadow-[10px_10px_0_0_#000] transition-all duration-300 ${
          isOpen
            ? "bg-[#FF90E8] rotate-90 scale-90"
            : "bg-[#26D0CE] hover:rotate-12"
        }`}
      >
        {isOpen ? (
          <X size={32} strokeWidth={3} className="text-black" />
        ) : (
          <QrCode size={32} strokeWidth={3} className="text-black" />
        )}
      </button>
    </div>
  );
